...
using namespace std;
constexpr int diameter=4,n_thread=1;
int choose_thres;
mt19937 rnd(0);
...
struct plan_t
{
	...
	double calc_dist(int i,const state &st)
	{
		...
		auto single_source=[&](int s)->vector<double>
		{
			...
			{
				dis[g.u]=min(dis[g.u],dis[g.v]+(3)*g.w);
				dis[g.v]=min(dis[g.v],dis[g.u]+(3)*g.w);
			}
			return dis;
		};
	...
	void perform(vector<state> &vs,int i,state st,int avoid=-1)
	{
		...
		auto single_source=[&](int s)->vector<pair<double,int>>
		{
			...
			{
				dis[g.u]=min(dis[g.u],{dis[g.v].first+(3)*g.w,g.v});
				dis[g.v]=min(dis[g.v],{dis[g.u].first+(3)*g.w,g.u});
			}
			return dis;
		};
...
void process1()
{
	...
	sum_gate+=(int)ans.second.ga.size();
//	cerr<<ans.first<<endl;
	oss<<"QREG q["<<n_qubit<<"];\n";
